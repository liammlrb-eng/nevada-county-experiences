...
## Generated documents (county pitch)

| File | Generated by | Purpose |
|---|---|---|
| `demo_pitch.pptx` | `generate_slides.py` | 15-slide pitch deck (16:9) |
| `demo_pitch.pdf` | `generate_demo_pdf.py` | Long-form PDF version of the pitch |
| `operator_guide.pdf` | `generate_operator_guide.py` | Day-to-day operator manual |

All three generators auto-mirror their output into `pitch_resources/`
after each run, so that folder always contains the current bundle.

## `pitch_resources/` folder

A self-contained bundle for sharing with Claude.ai chat (or attaching to
emails). Drag-and-drop the whole folder into a new chat to give Claude
full context on the pitch.

- `pitch_strategy_brief.md` — paste-into-Claude.ai brief for stakeholder strategy work
- `deck_outline.md` — slide-by-slide markdown outline of the .pptx
- `project_summary.md` — one-page platform overview
- `audience_map_template.md` — empty stakeholder map to fill in after a discovery call
- `demo_pitch.pptx`, `demo_pitch.pdf`, `operator_guide.pdf` — auto-mirrored binary snapshots
